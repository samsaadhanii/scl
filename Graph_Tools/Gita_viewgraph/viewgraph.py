...
def read_render(ex_file):
    ...
    headers = ['shloka id','word','morph in context','rel' ]

    data = pandas.read_excel(ex_file, usecols='C,D,G,Q', dtype=str,
                             names=headers, engine='openpyxl')
    data.dropna(how='all', inplace=True)
    data.replace(r'^\s+$', numpy.nan, regex=True, inplace=True)
    for i, fields in data.iterrows():
        if type(fields['shloka id']) != str or \
                not re.match(r'\d+', fields['shloka id']):
