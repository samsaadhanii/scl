...
if ( $out_encoding eq "DEV" ){
  ...
  $my_converter_wxHindi="$SCLINSTALLDIR/converters/ri | $SCLIINSTALLDIR/converters/iscii2utf8.py 1";
      ...
      <br /> <font color=\"red\">Below we give the step by step analysis of the input. By default only three rows with the original Sanskrit sentence, relevant morph analysis and the Hindi translation are shown. <br/> You can hide/open other rows using the <font color=\"green\">Show/Hide Rows button</font> at the bottom. The color of each cell indicates the vibhakti / category of the word. Details of color coding are available <a target=\"_blank\" href=\"/scl/SHMT/anu_help.html#sec1.4\">here</a>. <br /> If you bring the cursor on the sentence ID, you will see the <font color=\"green\">parsed tree (Kaaraka Analysis)</font> for the given sentence. Clicking on this link will show you all the possible parses. <br /> Finally each word is linked to the <font color=\"green\">Apte, Monier Williams, Heritage dictionary and Amarakosha</font>. The link is available along with the morph analysis displayed in row C and E.</font><br /> <br />";
