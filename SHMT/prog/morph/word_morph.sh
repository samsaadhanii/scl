...
$LTPROCBIN/lt-proc -c $BIN_PATH/samboXana.bin < $TMP_FILES_PATH/tmpsamb | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/\$$//' > $TMP_FILES_PATH/tmp_n2
$LTPROCBIN/lt-proc -c $BIN_PATH/all_but_samboXana_morf.bin < $TMP_FILES_PATH/tmpany | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/\$$//;s/[a-zA-Z]+<vargaH:sa-pU-pa><lifgam:[^>]+><level:0>//g;s/>([a-zA-Z]+)</><kqw_pratipadika:\1></g;' > $TMP_FILES_PATH/tmp_n3
