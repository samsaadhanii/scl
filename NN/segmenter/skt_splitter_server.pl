...
if(!$daemon_running){
   ...
   if (-s "$GlblVar::TFPATH/skt_splitter_daemonid"){
    ...
    #system("nohup $GlblVar::CGIDIR/$GlblVar::SCL_CGI/NN/segmenter/skt_splitter_server.sh > /dev/null &");
    system("nice $GlblVar::CGIDIR/$GlblVar::SCL_CGI/NN/segmenter/skt_splitter_server.sh > /dev/null &");
