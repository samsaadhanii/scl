...
#     $cmd = "$GlblVar::TIMEOUT $SCLINSTALLDIR/MT/prog/shell/anu_skt_hnd.sh $SCLINSTALLDIR $GraphvizDot in$pid $tmp_file_path hi $script $sandhi $morph $parse $text_type NOECHO $LTPROCBIN $MYPYTHONPATH 2>> $tmp_file_path/tmp_in$pid/err$pid;";
     $cmd = "$GlblVar::TIMEOUT $SCLINSTALLDIR/MT/prog/shell/anu_skt_hnd.sh $SCLINSTALLDIR $GraphvizDot in$pid $tmp_file_path hi $script $morph $parse $text_type /cgi-bin/$GlblVar::HERITAGE_CGI $LTPROCBIN $MYPYTHONPATH 2>> $tmp_file_path/tmp_in$pid/err$pid;";
