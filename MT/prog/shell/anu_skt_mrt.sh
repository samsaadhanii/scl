...
mar_gen(){
    ...
    $ANU_MT_PATH/map/mar/add_dict_mng.pl $CGIDIR/$SCL_CGI $ANU_MT_PATH/../data mr < /tmp/12.txt > /tmp/34.txt 
    $ANU_MT_PATH/map/mar/lwg_avy_avy.pl $CGIDIR/$SCL_CGI $ANU_MT_PATH/../data mr  < /tmp/34.txt  |\
    $ANU_MT_PATH/mar/sent_gen/agreement.pl $ANU_MT_PATH/../data $ANU_MT_PATH/mar/sent_gen > /tmp/56.txt 
    $ANU_MT_PATH/mar/word_gen/call_gen.pl $CGIDIR/$SCL_CGI $LTPROCBIN < /tmp/56.txt > /tmp/78.txt 
    $ANU_MT_PATH/interface/modify_mo_for_display.pl $CGIDIR/$SCL_CGI  < /tmp/78.txt > $temp_files_path/ttt
