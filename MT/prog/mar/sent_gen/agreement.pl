...
close(TMP);
open(TMP,"$Data_Path/hi/ne_excptnl_verbs.txt") || die "Can't open  $Data_Path/hi/ne_excptnl_verbs.txt for reading";
while(<TMP>) {
chomp;
$key = $_;
$EXCPT_NE{$key}=1;
...
foreach $w_ana (@wrd_ana){
  ...
  # Since the map file for adj is not yet modified, this is commented
# Example: kqwam karma smara
  #&chunk_viSeRaNa();
