...
function generate_kqw_forms(encod,vb,upasarga,outencoding){
  window.open('/cgi-bin/$GlblVar::SCL_CGI/skt_gen/kqw/kqw_gen.cgi?outencoding='+outencoding+'&encoding='+encod+'&vb='+vb+'&upasarga='+upasarga+'','popUpWindow','height=500,width=400,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes').focus();
}\n
