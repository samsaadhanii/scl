...
function generate_any_noun_forms(encod,prAwi,lifga,jAwi,outencoding,level){
  window.open('/cgi-bin/scl/skt_gen/noun/noun_gen.cgi?encoding='+encod+'&rt='+prAwi+'&gen='+lifga+'&jAwi='+jAwi+'&outencoding='+outencoding+'&level='+level+'','popUpWindow','height=500,width=400,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes').focus();
}
