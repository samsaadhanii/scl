...
<xsl:template match="/rt">
  ...
  <form action="" onsubmit="goto_section(); return false;">
    <p>
      ...
      <xsl:text disable-output-escaping="yes"> &lt;a href="/scl/MT/DEMO/tmp_anu_dir/table.xlsx"&gt;Save Annotation&lt;/a&gt;</xsl:text>
...
<!-- corresponding to morph analysis shown in row 3, we get all possible relations -->
           <xsl:text disable-output-escaping="yes"> &lt;a href="/cgi-bin/scl/MT/prog/interface/show_graph.cgi?filename=tmp_anu_dir/&amp;sentnum=</xsl:text>
	    <xsl:value-of select="$fig_sent_name"/>
            <xsl:text disable-output-escaping="yes">" </xsl:text>
            <xsl:text disable-output-escaping="yes"> onmouseover="Tip('&lt;img src=/scl/MT/DEMO/tmp_anu_dir/</xsl:text>
	    <xsl:value-of select="$fig_sent_name"/> 
            <xsl:text disable-output-escaping="yes">.svg &gt;' ,FONTSIZE,'18pt',HEIGHT,400,WIDTH,1200,STICKY,true,CLOSEBTN,true)" onmouseout="UnTip()"&gt;</xsl:text>
	    </xsl:if>
            <xsl:if test="$row_no=1">
            <xsl:text disable-output-escaping="yes"> &lt;a href="/cgi-bin/scl/MT/prog/interface/call_parser_summary.cgi?filename=tmp_dir_scrpt&amp;rel=''&amp;sentnum=</xsl:text>
